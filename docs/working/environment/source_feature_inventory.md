...
| Bonus-modified movement | source-read | Speed changes alter turn rate. Inverse flips turn. Straight-angle bonus applies one angular step then clears turn. | Source maps only. | `inverse_while_turning`; `speed_bonus_turn_rate`; `straight_angle_existing_turn`. |
...
| Death event killer fields | Python-verified | Wall killer is null. Body killer is body owner. `old` comes from body age. | Body canaries verify body killer and `old:false`; `source_body_old_metadata_batch.json` verifies one seeded old-body `old:true` event; wall fixtures verify wall death. | Wall/body/self event comparison. |
| `old` flag | Python-verified | `AvatarBody.oldAge = 2000`; it affects only emitted death event, not collision. | `source_body_old_opponent_overlap_kills_step.json` in the separate `source_body_old_metadata_batch.json` verifies `age_ms=2000` emits `old:true` on body death metadata. Current coverage is one old seeded opponent-body kill. | Add a young/old boundary pair only if broader metadata coverage is needed. |
