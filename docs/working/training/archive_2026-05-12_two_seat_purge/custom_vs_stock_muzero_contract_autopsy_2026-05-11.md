...
| `to_play` / player semantics | Stock Atari is effectively single-agent / not-board-game. CurvyTron-as-Pong should use scalar ego rows with `to_play=-1`; LightZero board-game player ids are `1/2`, not CurvyTron public ids `0/1`. | Some custom two-seat notes passed `to_play=player_id` for each active seat. Treat that as suspect, not a precedent. | Passing `0/1` player ids may trigger assumptions meant for turn-based board games or value sign handling. This can silently corrupt value targets. | For native-compatible CurvyTron replay, use `to_play=-1` unless a real board-game-style contract with `1/2` ids is deliberately implemented and tested. |
