...
| Manifest checkpoint-ref existence audit | Main | Active | Add a prelaunch audit tool/test that collects initial checkpoint refs, assignment frozen refs, and mixture frozen refs from a manifest and fails if refs are missing from the selected v2 runs root/Volume. This prevents old snapshots from looking launchable while pointing at files that do not exist in `curvyzero-runs-v2`. |
...
| Restart gate for next training | Main | Unblocked by wiring proof; manifest audit still active | The recreated all-v2 loop is proven at canary scale, the focused launch/promotion regression bundle passed (`26 passed`), and the broader E2E-adjacent slice passed (`343 passed, 24 skipped`). Before a real batch, build/audit the exact restart18 manifest from an explicit all-v2 snapshot and choose fresh tournament/rating ids. |
...
| Production source strategy | Rawls + Main | Active | Rawls is auditing whether the clean next source should be rematerialized old champions into v2, migrated old leaderboard plus checkpoint files, or a fresh grow-from-canary lane. Main should not launch a real restart18 manifest before this decision is written down. |
