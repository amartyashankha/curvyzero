...
| `next_matrix_manifest_design.md` old 48-row sketch | Looks like a runnable matrix with stale reward names and bundled no-trail/immortal rows | Done: added stop banner, marked old tables/flags archival, and restated current blank-canvas/no-outcome lane |
| `next_overnight_matrix_plan.md` broad axes | Could imply reward/opponent Cartesian launch is ready | Done: narrowed active plan to blank-canvas anchor, repeat copies, matched renders, and gated scripted/random lanes |
| `scripts/build_curvytron_stock_train_manifest.py` | Emits historical fixed/frozen rows with stale reward/opponent/seed schema | Done: historical-only and fails closed unless `--allow-historical-matrix` is passed; output marks `current_launch_approved=false` |
