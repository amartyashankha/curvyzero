...
cdef extern from "lib/cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        void expand_a3(int to_play, int current_latent_state_index, int batch_index, float value_prefixs, const float *policy_logits)
    ...
    void cset_deterministic_tie_breaking(bint enabled)
    ...
    void cbatch_backpropagate_flat_a3(int current_latent_state_index, float discount_factor, const float *value_prefixs, const float *values, const float *policies_a3,
                               CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, int num)
