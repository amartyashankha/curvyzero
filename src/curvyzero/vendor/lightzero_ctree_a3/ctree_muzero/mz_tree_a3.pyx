...
def batch_backpropagate_flat_a3(int current_latent_state_index, float discount_factor,
                                float[::1] value_prefixs, float[::1] values,
                                float[:, ::1] policies,
                                MinMaxStatsList min_max_stats_lst, ResultsWrapper results):
    cdef int num = value_prefixs.shape[0]
    if values.shape[0] != num:
        raise ValueError("values must have the same length as value_prefixs")
    if policies.shape[0] != num or policies.shape[1] != 3:
        raise ValueError("policies must have shape [N,3]")
    if policies.strides[0] != 3 * sizeof(float):
        raise ValueError("policies must be C-contiguous float32 [N,3]")
    if results.cresults.num != num:
        raise ValueError("results num must match value_prefixs length")
    if results.cresults.virtual_to_play_batchs.size() != num:
        raise ValueError("results must come from batch_traverse before flat backpropagate")
    if num <= 0:
        return

    cbatch_backpropagate_flat_a3(current_latent_state_index, discount_factor,
                                 &value_prefixs[0], &values[0], &policies[0, 0],
                                 min_max_stats_lst.cmin_max_stats_lst, results.cresults,
                                 num)

def set_deterministic_tie_breaking(bint enabled):
    cset_deterministic_tie_breaking(enabled)
