...
namespace tree
{
    bool g_deterministic_tie_breaking = false;

    void cset_deterministic_tie_breaking(bool enabled)
    {
        g_deterministic_tie_breaking = enabled;
    }
    ...
    void CNode::expand_a3(int to_play, int current_latent_state_index, int batch_index, float reward, const float *policy_logits)
    {
        /*
        Fixed-action-count variant for the CurvyZero profile-only CTree ABI.
        It preserves the same softmax/child creation semantics as expand(...)
        while avoiding one std::vector<float> allocation per row.
        */
        this->to_play = to_play;
        this->current_latent_state_index = current_latent_state_index;
        this->batch_index = batch_index;
    ...
    void cbatch_backpropagate_flat_a3(int current_latent_state_index, float discount_factor, const float *rewards, const float *values, const float *policies_a3, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, int num)
    {
        /*
        Fixed A=3 flat-payload variant used by CurvyZero profiling.
        The tree semantics match cbatch_backpropagate; only the payload ABI is
        flat arrays instead of Python lists -> nested vectors.
        */
        for (int i = 0; i < num; ++i)
        {
            int to_play = results.virtual_to_play_batchs[i];
            results.nodes[i]->expand_a3(to_play, current_latent_state_index, i, rewards[i], policies_a3 + 3 * i);
            cbackpropagate(results.search_paths[i], min_max_stats_lst->stats_lst[i], to_play, values[i], discount_factor);
        }
    }
    ...
    int cselect_child(CNode *root, tools::CMinMaxStats &min_max_stats, int pb_c_base, float pb_c_init, float discount_factor, float mean_q, int players)
    {
        ...
        */
        ...
        {
            if (g_deterministic_tie_breaking)
            {
                action = max_index_lst[0];
            }
            else
            {
