"""Modal wrapper for LightZero dummy Pong MCTS eval-mode scorecards.

Run from the repository root:

    uv run --extra modal modal run \
      -m curvyzero.infra.modal.lightzero_dummy_pong_mcts_scoreboard_attempt \
      --checkpoints lightzero:iter8=ref:training/lightzero-dummy-pong/RUN_ID/checkpoints/lightzero/iteration_8.pth.tar \
      --episodes 20 \
      --seed 0

This scores ``MuZeroPolicy.eval_mode.forward`` with MCTS. It is intentionally
separate from the direct policy-head greedy scoreboard.
"""
...
APP_NAME = "curvyzero-lightzero-dummy-pong-mcts-scoreboard"
TASK_ID = "lightzero-dummy-pong"
DEFAULT_EVAL_ID = "mcts-scoreboard"
...
def _baseline_policy_args_from_text(baseline_policies: str | None) -> list[str] | None:
    if baseline_policies is None:
        return None
    values = [
        item.strip()
        for item in baseline_policies.replace("\n", ",").split(",")
        if item.strip()
    ]
...
def score_lightzero_dummy_pong_mcts(
    checkpoints: str,
    episodes: int = 20,
    seed: int = 0,
    ...
    baseline_policies: str | None = None,
) -> dict[str, Any]:
    import curvyzero.training.dummy_pong_eval as eval_module
    from scripts.run_dummy_pong_lightzero_mcts_checkpoint_scoreboard import (
    ...
    summary = eval_module.run_dummy_pong_eval(
        episodes=episodes,
        seed=seed,
        output_dir=eval_dir,
        lightzero_mcts_checkpoint_policies=resolved_checkpoint_args,
        lightzero_feature_mode=feature_mode,
        lightzero_max_env_step=max_env_step,
        lightzero_num_simulations=num_simulations,
        pong_reset_profile=pong_reset_profile,
        pong_reset_pressure_agent=pong_reset_pressure_agent,
        paired_seats=paired_seats,
        baseline_policy_names=_baseline_policy_args_from_text(baseline_policies),
    )
    ...
    summary["modal_eval"] = {
        "schema": "curvyzero_modal_lightzero_mcts_scoreboard/v0",
        "app_name": APP_NAME,
        ...
        "independent_eval_max_steps": summary.get("config", {}).get("max_steps"),
        "plain_language": {
            "proves": "Independent LightZero checkpoint scoring through eval-mode MCTS.",
            "does_not_prove": "That the trained policy is useful or non-constant.",
        },
    }
    ...
    result = {
        "schema": "curvyzero_modal_lightzero_mcts_scoreboard_result/v0",
        "app_name": APP_NAME,
        "volume_name": VOLUME_NAME,
        "output_refs": {
...
def main(
    ...
    started = time.perf_counter()
    result = score_lightzero_dummy_pong_mcts.remote(
