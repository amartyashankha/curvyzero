"""Tiny official LightZero Connect4 MuZero trainer smoke on Modal.
...
Run the tiny remote train smoke:
    ...
    uv run --extra modal modal run -m curvyzero.infra.modal.lightzero_connect4_tiny_train_smoke --mode train

This uses the stock LightZero Connect4 MuZero bot-mode config. Connect4 has a
delayed terminal reward and the official config uses td_steps=21 with
discount_factor=1 for final-outcome targets.

LightZero==0.2.0 on Modal does not ship zoo.board_games.connect4, so this
mounts the local official /tmp/lightzero-src/zoo snapshot while keeping the
trainer and LightZero package install stock.
"""
...
APP_NAME = "curvyzero-lightzero-connect4-tiny-train-smoke"
TASK_ID = "lightzero-official-connect4"
VOLUME_NAME = "curvyzero-runs"
RUNS_MOUNT = Path("/runs")
REMOTE_ROOT = Path("/repo")
LIGHTZERO_ZOO_ROOT = REMOTE_ROOT / "lightzero-zoo"
...
image = (
    ...
    .env({"PYTHONPATH": f"{LIGHTZERO_ZOO_ROOT}:{REMOTE_ROOT / 'src'}"})
    .add_local_dir(Path("/tmp/lightzero-src/zoo"), remote_path=str(LIGHTZERO_ZOO_ROOT / "zoo"), copy=True)
...
def _extract_surface(main_config: Any, create_config: Any, *, max_env_step: int) -> dict[str, Any]:
    return {
        "env_id": main_config["env"].get("env_id", "Connect4"),
...
def _patched_connect4_configs(
    *,
    ...
    module_name = "zoo.board_games.connect4.config.connect4_muzero_bot_mode_config"
    ...
    patches = [
        _set_path(
            main_config,
            ("exp_name",),
            str(Path("/tmp") / "curvyzero-lightzero-connect4-tiny" / f"seed-{seed}"),
        ),
        ...
        _set_or_add_path(main_config, ("env", "env_id"), "Connect4"),
        _set_or_add_path(main_config, ("env", "bot_action_type"), "rule"),
        _set_path_if_present(main_config, ("env", "manager", "shared_memory"), False),
        _set_path(main_config, ("policy", "cuda"), False),
        _set_or_add_path(main_config, ("policy", "model", "reward_support_range"), (-300.0, 301.0, 1.0)),
        _set_or_add_path(main_config, ("policy", "model", "value_support_range"), (-300.0, 301.0, 1.0)),
        _set_or_add_path(main_config, ("policy", "model_path"), None),
        ...
        _set_path(main_config, ("policy", "game_segment_length"), 21),
        _set_path(main_config, ("policy", "td_steps"), 21),
        _set_or_add_path(main_config, ("policy", "num_unroll_steps"), 3),
...
def _validate_patched_surface(surface: dict[str, Any], *, max_train_iter: int, mode: str) -> list[str]:
    ...
    for key, value in expected.items():
        if surface[key] != value:
            problems.append(f"patched Connect4 surface {key}={surface[key]!r}, expected {value!r}")
    caps = {
    ...
    for key, ceiling in caps.items():
        if int(surface[key]) > ceiling:
            problems.append(f"patched Connect4 cap {key}={surface[key]!r} exceeds {ceiling}")
    if tuple(surface["reward_support_range"]) != (-300.0, 301.0, 1.0):
        problems.append(f"reward_support_range={surface['reward_support_range']!r}, expected (-300, 301, 1)")
    if tuple(surface["value_support_range"]) != (-300.0, 301.0, 1.0):
        problems.append(f"value_support_range={surface['value_support_range']!r}, expected (-300, 301, 1)")
...
def _run_lightzero_connect4_tiny_smoke(
    *,
    mode: str,
    seed: int,
    max_env_step: int,
    ...
    run_id = run_id or runs.new_run_id("lz-official-connect4")
    ...
    config = {
        "job_kind": "lightzero_official_connect4_muzero_tiny_train",
        "official_example": "zoo.board_games.connect4.config.connect4_muzero_bot_mode_config",
    ...
    patched = _patched_connect4_configs(
        seed=seed,
        max_env_step=max_env_step,
        collector_env_num=collector_env_num,
        evaluator_env_num=evaluator_env_num,
    ...
    result = {
        "ok": not problems,
        "label": "stock LightZero Connect4 MuZero tiny trainer smoke",
        "task_id": TASK_ID,
        "mode": mode,
        "run_id": run_id,
        "attempt_id": attempt_id,
        ...
        "reward_sparsity": {
            "source": "zoo.board_games.connect4.envs.connect4_env.Connect4Env._player_step",
...
def lightzero_connect4_tiny_train_smoke(
    mode: str = "dry",
    seed: int = 0,
    ...
    return _run_lightzero_connect4_tiny_smoke(
        mode=mode,
        seed=seed,
        max_env_step=max_env_step,
        max_train_iter=max_train_iter,
...
def main(
    ...
    result = lightzero_connect4_tiny_train_smoke.remote(
        mode=mode,
        seed=seed,
        max_env_step=max_env_step,
        max_train_iter=max_train_iter,
